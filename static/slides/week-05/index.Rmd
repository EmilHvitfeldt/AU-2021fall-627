...
```{r, echo=FALSE}
library(recipes)
recipe(~MS_Zoning, data = ames) %>%
  step_dummy(MS_Zoning, one_hot = TRUE) %>%
  prep() %>%
  bake(new_data = NULL) %>%
  dplyr::glimpse()
```
...
library(recipes)
recipe(~MS_Zoning, data = ames) %>%
  step_dummy(MS_Zoning, one_hot = FALSE) %>%
  prep() %>%
  bake(new_data = NULL) %>%
  dplyr::glimpse()
```
...
```{r, echo=FALSE}
library(recipes)
recipe(~Neighborhood, data = ames) %>%
  step_dummy(Neighborhood, one_hot = FALSE) %>%
  prep() %>%
  bake(new_data = NULL) %>%
  dplyr::glimpse()
```
...
ames %>%
  count(Neighborhood) %>%
  mutate(Neighborhood = forcats::fct_rev(Neighborhood)) %>%
...
library(dplyr)
ames %>%
  mutate(Neighborhood = forcats::fct_lump_prop(Neighborhood, prop = 0.01)) %>%
  count(Neighborhood) %>%
  ggplot(aes(n, Neighborhood)) +
  geom_col()
```
